...
int armv8m_trace_tpiu_config(struct target *target)
{
	...
	if (trace_config->config_type == TRACE_CONFIG_TYPE_INTERNAL_V8M)
		target_register_timer_callback(armv8m_poll_trace, 1,
...
COMMAND_HANDLER(handle_tpiu_config_command)
{
	...
	if (!strcmp(CMD_ARGV[cmd_idx], "disable")) {
		...
		if (CMD_ARGC != cmd_idx) {
			...
			if (armv8m->trace_config.config_type != TRACE_CONFIG_TYPE_INTERNAL_V8M) {
