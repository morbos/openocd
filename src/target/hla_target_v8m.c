...
#define ARMV8M_SCS_DCRSR	DCB_DCRSR
#define ARMV8M_SCS_DCRDR	DCB_DCRDR
		...
		uint32_t num, uint32_t *value)
{
	 ...
	 * in the v8m header match the Cortex-MX3 Debug Core Register
	 * Selector values for R0..R15, xPSR, MSP, and PSP + others
	 */
	switch (num) {
	case 0 ... 18:
	...
	case ARMV8M_FPSCR:
		...
		retval = target_write_u32(target, ARMV8M_SCS_DCRSR, 33);
		if (retval != ERROR_OK)
			return retval;
		retval = target_read_u32(target, ARMV8M_SCS_DCRDR, value);
		if (retval != ERROR_OK)
			return retval;
	...
	case ARMV8M_S0 ... ARMV8M_S31:
		/* Floating-point Status and Registers */
		retval = target_write_u32(target, ARMV8M_SCS_DCRSR, num-ARMV8M_S0+64);
		if (retval != ERROR_OK)
			return retval;
		retval = target_read_u32(target, ARMV8M_SCS_DCRDR, value);
		if (retval != ERROR_OK)
		...
		uint32_t num, uint32_t value)
{
	...
	case ARMV8M_FPSCR:
		/* Floating-point Status and Registers */
		retval = target_write_u32(target, ARMV8M_SCS_DCRDR, value);
		if (retval != ERROR_OK)
			return retval;
		retval = target_write_u32(target, ARMV8M_SCS_DCRSR, 33 | (1<<16));
		if (retval != ERROR_OK)
			return retval;
	...
	case ARMV8M_S0 ... ARMV8M_S31:
		...
		if (retval != ERROR_OK)
			return retval;
		retval = target_write_u32(target, ARMV8M_SCS_DCRSR, (num-ARMV8M_S0+64) | (1<<16));
		if (retval != ERROR_OK)
			return retval;
	...
	case ARMV8M_CONTROL:
		...
		case ARMV8M_CONTROL_S:
			 ...
			 * it was removed from r1 docs, but still works.
			 */
		        adapter->layout->api->read_reg(adapter->handle, 34, &reg);
			...
			switch (num) {
		        ...
		        adapter->layout->api->write_reg(adapter->handle, 34, reg);
		...
		case ARMV8M_CONTROL_NS:
			 ...
			 * it was removed from r1 docs, but still works.
			 */
		        adapter->layout->api->read_reg(adapter->handle, 35, &reg);
			...
			adapter->layout->api->write_reg(adapter->handle, 35, reg);
...
static int adapter_init_arch_info(struct target *target,
				       struct cortex_mX3_common *cortex_mX3,
				       struct jtag_tap *tap)
{
	struct armv8m_common *armv8m;

	LOG_DEBUG("%s", __func__);

	armv8m = &cortex_mX3->armv8m;
	armv8m_init_arch_info(target, armv8m);

	armv8m->load_core_reg_u32 = adapter_load_core_reg_u32;
	armv8m->store_core_reg_u32 = adapter_store_core_reg_u32;

	armv8m->examine_debug_reason = adapter_examine_debug_reason;
	armv8m->stlink = true;
		...
		Jim_Interp *interp)
{
	...
	adapter_init_arch_info(target, cortex_mX3, target->tap);
...
{
	struct armv8m_common *armv8m = target_to_armv8m(target);
	int num_regs = armv8m->arm.core_cache->num_regs;

	for (int i = 0; i < num_regs; i++) {

		struct reg *r = &armv8m->arm.core_cache->reg_list[i];
		if (!r->valid)
			armv8m->arm.read_core_reg(target, r, i, ARM_MODE_ANY);
	}
...
static int adapter_poll(struct target *target)
{
	...
	if (state == TARGET_HALTED) {
		...
		LOG_DEBUG("halted: PC: 0x%08" PRIx32, buf_get_u32(armv8m->arm.pc->value, 0, 32));
	}
		...
		int debug_execution)
{
	...
	res = target_write_u32(target, DCB_DEMCR, TRCENA | armv8m->demcr);
	if (res != ERROR_OK)
		return res;
		...
		target_addr_t address, int handle_breakpoints)
{
	...
	LOG_INFO("halted: PC: 0x%08" PRIx32, buf_get_u32(armv8m->arm.pc->value, 0, 32));
...
struct target_type hla_target_v8m = {
	.name = "hla_target_v8m",
	.deprecated_name = "stm32_stlink_v8m",
