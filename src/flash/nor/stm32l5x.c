...
struct stm32l5_flash_bank {
	struct stm32l5_options option_bytes;
	int probed;
};
...
static inline uint32_t stm32l5_get_flash_reg(struct flash_bank *bank, uint32_t reg, uint32_t stat_cr)
{
...
static inline int stm32l5_get_flash_status(struct flash_bank *bank, uint32_t *status)
{
	...
	target, stm32l5_get_flash_reg(bank, STM32_FLASH_NSSR, 1), status);
}
...
static int stm32l5_wait_status_busy(struct flash_bank *bank, int timeout)
{
	...
	if (status & FLASH_ERROR) {
		 ...
		 * retval
		 */
		target_write_u32(target, stm32l5_get_flash_reg(bank, STM32_FLASH_NSSR, 1),
				status & FLASH_ERROR);
	}
	return retval;
}

static int stm32l5_unlock_reg(struct flash_bank *bank, struct target *target)
{
	uint32_t ctrl;
	 ...
	 */
	int retval = target_read_u32(target, stm32l5_get_flash_reg(bank, STM32_FLASH_NSCR, 1), &ctrl);
	if (retval != ERROR_OK)
		return retval;
	...
	retval = target_write_u32(target, stm32l5_get_flash_reg(bank, STM32_FLASH_NSKEYR, 1), KEY1);
	if (retval != ERROR_OK)
		return retval;

	retval = target_write_u32(target, stm32l5_get_flash_reg(bank, STM32_FLASH_NSKEYR, 1), KEY2);
	if (retval != ERROR_OK)
		return retval;

	retval = target_read_u32(target, stm32l5_get_flash_reg(bank, STM32_FLASH_NSCR, 1), &ctrl);
	if (retval != ERROR_OK)
		return retval;
...
static int stm32l5_unlock_option_reg(struct flash_bank *bank, struct target *target)
{
	uint32_t ctrl;
	...
	retval = target_write_u32(target, stm32l5_get_flash_reg(bank, STM32_FLASH_OPTKEYR, 0), OPTKEY1);
	if (retval != ERROR_OK)
		return retval;

	retval = target_write_u32(target, stm32l5_get_flash_reg(bank, STM32_FLASH_OPTKEYR, 0), OPTKEY2);
	if (retval != ERROR_OK)
		return retval;
...
static int stm32l5_read_options(struct flash_bank *bank)
{
	uint32_t optiondata;
	struct stm32l5_flash_bank *stm32l5_info = NULL;
	struct target *target = bank->target;

	stm32l5_info = bank->driver_priv;

	/* read current option bytes */
	int retval = target_read_u32(target, stm32l5_get_flash_reg(bank, STM32_FLASH_OPTR, 0), &optiondata);
	if (retval != ERROR_OK)
		return retval;

	stm32l5_info->option_bytes.user_options = (optiondata >> 8) & 0x3ffff;
	stm32l5_info->option_bytes.RDP = optiondata & 0xff;

	retval = target_read_u32(target, stm32l5_get_flash_reg(bank, STM32_FLASH_WRP1AR, 0), &optiondata);
	if (retval != ERROR_OK)
		return retval;
	stm32l5_info->option_bytes.wpr1a_start =  optiondata         & 0xff;
	stm32l5_info->option_bytes.wpr1a_end   = (optiondata >> 16)  & 0xff;

	retval = target_read_u32(target, STM32_FLASH_WRP2AR, &optiondata);
	if (retval != ERROR_OK)
		return retval;
	stm32l5_info->option_bytes.wpr2a_start =  optiondata         & 0xff;
	stm32l5_info->option_bytes.wpr2a_end   = (optiondata >> 16)  & 0xff;

	retval = target_read_u32(target, STM32_FLASH_WRP1BR, &optiondata);
	if (retval != ERROR_OK)
		return retval;
	stm32l5_info->option_bytes.wpr1b_start =  optiondata         & 0xff;
	stm32l5_info->option_bytes.wpr1b_end   = (optiondata >> 16)  & 0xff;

	retval = target_read_u32(target, STM32_FLASH_WRP2BR, &optiondata);
	if (retval != ERROR_OK)
		return retval;
	stm32l5_info->option_bytes.wpr2b_start =  optiondata         & 0xff;
	stm32l5_info->option_bytes.wpr2b_end   = (optiondata >> 16)  & 0xff;

	if (stm32l5_info->option_bytes.RDP != 0xAA)
		LOG_INFO("Device Security Bit Set");

	return ERROR_OK;
}

static int stm32l5_write_options(struct flash_bank *bank)
{
	struct stm32l5_flash_bank *stm32l5_info = NULL;
	struct target *target = bank->target;
	uint32_t optiondata;
	...
	int retval = stm32l5_unlock_option_reg(bank, target);
	if (retval != ERROR_OK)
		return retval;
...
static int stm32l5_protect_check(struct flash_bank *bank)
{
	...
	int retval = stm32l5_read_options(bank);
	if (retval != ERROR_OK) {
	...
	for (int i = 0; i < bank->num_sectors; i++) {
		if (i < stm32l5_info->option_bytes.bank_b_start) {
			if (((i >= stm32l5_info->option_bytes.wpr1a_start) &&
				 (i <= stm32l5_info->option_bytes.wpr1a_end)) ||
				((i >= stm32l5_info->option_bytes.wpr2a_start) &&
				 (i <= stm32l5_info->option_bytes.wpr2a_end)))
			...
			if (((snb >= stm32l5_info->option_bytes.wpr1b_start) &&
				 (snb <= stm32l5_info->option_bytes.wpr1b_end)) ||
				((snb >= stm32l5_info->option_bytes.wpr2b_start) &&
				 (snb <= stm32l5_info->option_bytes.wpr2b_end)))
				bank->sectors[i].is_protected = 1;
			else
				bank->sectors[i].is_protected = 0;
		}
...
static int stm32l5_lift_watermarks(struct flash_bank *bank)
{
    int retval;
    struct target *target = bank->target;
    /* 
    ...
    stm32l5_unlock_option_reg(bank, target);
...
static int stm32l5_erase(struct flash_bank *bank, int first, int last)
{
	...
	int retval;
	retval = stm32l5_unlock_reg(bank, target);
	if (retval != ERROR_OK)
		return retval;
	...
	retval = stm32l5_lift_watermarks(bank);
	if (retval != ERROR_OK)
	    return retval;
	...
	for (i = first; i <= last; i++) {
		...
		retval = target_write_u32(target,
				stm32l5_get_flash_reg(bank, STM32_FLASH_NSCR, 1), erase_flags);
		if (retval != ERROR_OK)
			return retval;
	...
	retval = target_write_u32(
		target, stm32l5_get_flash_reg(bank, STM32_FLASH_NSCR, 1), FLASH_LOCK);
	if (retval != ERROR_OK)
		return retval;
...
static int stm32l5_protect(struct flash_bank *bank, int set, int first, int last)
{
	...
	retval = stm32l5_write_options(bank);
	if (retval != ERROR_OK)
		return retval;
		...
		uint32_t offset, uint32_t count)
{
	...
	if(is_secure(address)) {
	    ...
	    // The loader stm32l5x.S has the NS def's so this... works. Uncommented, this
	...
	if (retval == ERROR_FLASH_OPERATION_FAILED) {
		...
		if (error != 0) {
			...
			/* Clear but report errors */
			target_write_u32(target, stm32l5_get_flash_reg(bank, STM32_FLASH_NSSR, 1), error);
			retval = ERROR_FAIL;
		}
	}
		...
		uint32_t offset, uint32_t count)
{
	...
	return target_write_u32(target, stm32l5_get_flash_reg(bank, STM32_FLASH_NSCR, 1), FLASH_LOCK);
}
...
static int stm32l5_probe(struct flash_bank *bank)
{
	...
	case 0x472:
		...
		break;
	default:
		LOG_WARNING("Cannot identify target as a STM32L5 family (%x).", device_id);
		return ERROR_FAIL;
	}
	...
	retval = target_read_u32(target, stm32l5_get_flash_reg(bank, STM32_FLASH_OPTR, 0), &options);

	/* only devices with < 1024 kiB may be set to single bank dual banks */
	if ((flash_size_in_kb == 1024) || !(options & OPT_DUALBANK))
		stm32l5_info->option_bytes.bank_b_start = 256;
	else
		stm32l5_info->option_bytes.bank_b_start = flash_size_in_kb << 9;
...
static int get_stm32l5_info(struct flash_bank *bank, char *buf, int buf_size)
{
	...
	switch (device_id) {
	case 0x472:
		device_str = "STM32L5x2";
		break;
	default:
		snprintf(buf, buf_size, "Cannot identify target as a STM32L5 (%x)\n", device_id);
		return ERROR_FAIL;
	}
...
COMMAND_HANDLER(stm32l5_handle_lock_command)
{
	...
	if (stm32l5_read_options(bank) != ERROR_OK) {
	...
	stm32l5_info->option_bytes.RDP = 0;

	if (stm32l5_write_options(bank) != ERROR_OK) {
...
COMMAND_HANDLER(stm32l5_handle_unlock_command)
{
	...
	stm32l5_info->option_bytes.RDP = 0xAA;
...
static int stm32l5_mass_erase(struct flash_bank *bank, uint32_t action)
{
	...
	retval = target_write_u32(
		target, stm32l5_get_flash_reg(bank, STM32_FLASH_NSCR, 1), action);
	if (retval != ERROR_OK)
		return retval;
	retval = target_write_u32(
		target, stm32l5_get_flash_reg(bank, STM32_FLASH_NSCR, 1),
		action | FLASH_STRT);
	if (retval != ERROR_OK)
		return retval;
